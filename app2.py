...
accounts = [
    ...
    {'apiKey': os.getenv('OKX_API_KEY_1'), 'secret': os.getenv('OKX_API_SECRET_1'), 'password': os.getenv('OKX_PASSWORD_1'), 'name': 'OKX_1', 'balance_want': 131, 'pos_size_usdt': 5.55},
    ...
    {'apiKey': os.getenv('OKX_API_KEY_4'), 'secret': os.getenv('OKX_API_SECRET_4'), 'password': os.getenv('OKX_PASSWORD_4'), 'name': 'OKX_4', 'balance_want': 103, 'pos_size_usdt': 4.3},
    ...
    {'apiKey': os.getenv('OKX_API_KEY_9'), 'secret': os.getenv('OKX_API_SECRET_9'), 'password': os.getenv('OKX_PASSWORD_9'), 'name': 'OKX_9', 'balance_want': 117, 'pos_size_usdt': 4.85},
...
<div class="update-form">
  <h3>Cập nhật Target Balance (balance_want)</h3>
  <label for="accIndexBW">Tài khoản:</label>
  <select id="accIndexBW">
    {% for i in range(11) %}
      <option value="{{ i }}">{{ accounts[i].name }} (Hiện tại: {{ accounts[i].balance_want }})</option>
    {% endfor %}
  </select>
  <br>
  <label for="newBalanceData" style="display:block; margin-top:10px;">Nhập Target Balance mới (USDT):</label>
  <input type="number" id="newBalanceData" step="0.1" placeholder="Ví dụ: 150.5" style="width: 100%; font-family: monospace; margin-top: 5px; padding: 8px; border: 1px solid #ccc; border-radius: 3px; box-sizing: border-box;" />
  <br>
  <button type="button" class="btn-confirm" onclick="updateBalanceWant()" style="margin-top: 15px;">Cập nhật Balance</button>
...
function updateBalanceWant() {
    ...
    const newBalance = document.getElementById("newBalanceData").value;
    const msg = document.getElementById("updateBWMsg");

    if(!newBalance.trim()) {
        msg.style.color = "red";
        msg.innerText = "Vui lòng nhập số Target Balance mới!";
        return;
    }
    ...
    fetch("/update_balance_want", {
        method: "POST",
        headers: { "Accept": "application/json", "Content-Type": "application/json" },
        body: JSON.stringify({ "accIndex": accIndex, "newBalance": newBalance })
    })
    .then(response => response.json())
    .then(data => {
        if(data.success) {
...
def raw_create_order(exchange, symbol, type_str, side, amount, params):
    """
    Gọi trực tiếp API v5 của OKX để tạo lệnh, bỏ qua hoàn toàn cơ chế của CCXT.
    - params chứa các biến phụ (VD: posSide, slTriggerPx, leverage,...)
    """
    time.sleep(0.5)  # Delay nhẹ để chống Spam/Rate limit
    
    instId = symbol.replace('/', '-').split(':')[0] + '-SWAP'
    
    # Các tham số cơ bản cho một lệnh thị trường (market) hoặc giới hạn (limit)
    payload = {
        'instId': instId,
    ...
    # Bổ sung position side
    if 'posSide' in params:
        payload['posSide'] = params['posSide']
        
    # Xử lý riêng cho lệnh Stop Loss (dạng stop_market)
    if type_str == 'stop_market' or 'stopLossPrice' in params:
        payload['ordType'] = 'conditional'
        payload['slTriggerPx'] = str(params.get('stopLossPrice'))
        payload['slOrdPx'] = '-1' # Lệnh kích hoạt SL sẽ được đẩy vào thị trường ở giá market
        # Đối với Stop Loss qua API gốc, thường bạn truyền thêm tham số phụ này
        if 'posSide' in params:
            payload['posSide'] = params['posSide']
            
    # Xử lý đòn bẩy nếu có
    if 'leverage' in params:
        try:
            exchange.private_post_account_set_leverage({
                'instId': instId,
                'mgnMode': params.get('tdMode', 'cross'),
                'lever': str(params['leverage']),
                'posSide': params.get('posSide', side)
            })
            time.sleep(0.2) # Chờ API cập nhật đòn bẩy
        except Exception:
            pass # Lờ đi nếu sàn báo đòn bẩy đã là mức đó
            
    # Gọi hàm riêng (private endpoint) của OKX
    response = exchange.private_post_trade_order(payload)
    
    if response and response.get('code') == '0':
        return response
    else:
        raise Exception(f"Raw API Error: {response}")
...
def set_stop_loss(exchange, symbol, position_side, amount, stop_price, acc_name, diff_pct):
    ...
    try:
        ...
        # Lỗi phát sinh, chuyển sang cơ chế Raw API
        error_msg = str(e)
        if "'<' not supported" in error_msg or "NoneType" in error_msg:
            try:
                log(f"🛠 [{acc_name}] CCXT lỗi khi đặt SL cho {token}. Dùng Raw API fallback...")
                raw_create_order(
                    exchange, symbol, 'stop_market', side, amount, 
                    {'stopLossPrice': stop_price, 'posSide': position_side}
                )
                log(f"{acc_name} | Set SL (Raw API) {token} {position_side.upper()} → {diff_pct:.1f}%")
                return True
            except Exception as raw_e:
                log(f"{acc_name} | Raw SL Error {token}: {str(raw_e)}")
                return False
        else:
            # Lỗi khác không liên quan đến CCXT NoneType
            log(f"{acc_name} | SL Error {token}: {error_msg}")
            return False
...
def close_all_token(exchange, acc_name):
    try:
        # === 1. LẤY DANH SÁCH VỊ THẾ CẦN ĐÓNG ===
        try:
            positions = exchange.fetch_positions(None, params={'type': 'swap'})
        except Exception as e:
            error_msg = str(e)
            if "'<' not supported" in error_msg or "NoneType" in error_msg:
                log(f"🛠 [{acc_name}] CCXT lỗi lấy vị thế Close All. Chuyển sang Raw API...")
                raw_pos = exchange.private_get_account_positions({'instType': 'SWAP'})
                positions = []
                if raw_pos and 'data' in raw_pos:
                    for p in raw_pos['data']:
                        contracts = float(p.get('pos', 0))
                        if contracts == 0: continue
                        ...
                        side_raw = p.get('posSide', 'long').lower()
                        if side_raw == 'net': 
                ...
                raise e # Nếu là lỗi mạng khác thì báo lỗi ra ngoài
                
        active = [p for p in positions if float(p['contracts']) > 0]
        
        # === 2. THỰC THI ĐÓNG TỪNG LỆNH ===
        for p in active:
            symbol = p['symbol']
            ...
            try:
                ...
                if "'<' not supported" in error_msg or "NoneType" in error_msg:
                    try:
                        log(f"🛠 [{acc_name}] CCXT lỗi đóng lệnh {symbol}. Chuyển sang Raw API...")
                        
                        # Dùng hàm raw_create_order vừa tạo ở bước trước
                        raw_resp = raw_create_order(
                            exchange, symbol, 'market', side, amount,
                            {'posSide': p['side']}
                        )
                        
                        ord_id = raw_resp['data'][0]['ordId'] if (raw_resp and 'data' in raw_resp) else 'Raw_OK'
                        log(f"{acc_name} | Close ALL (Raw API) {extract_token(symbol)} {p['side'].upper()} | Order: {ord_id}")
                    except Exception as raw_e:
                        log(f"{acc_name} | Raw Close Error {extract_token(symbol)}: {raw_e}")
                else:
    ...
    for symbol, change in wrapped_tokens:
        ...
        try:
            ...
            try:
                ...
                error_msg = str(e)
                if "'<' not supported" in error_msg or "NoneType" in error_msg:
                    log(f"🛠 CCXT lỗi Open {token_name}. Fallback Raw API...")
                    raw_resp = raw_create_order(
                        exchange, symbol, 'market', order_side, amount,
                        {'posSide': direction, 'tdMode': 'cross'}
                    )
                    order = {'id': raw_resp.get('data', [{}])[0].get('ordId', 'Raw_OK')}
                else:
                    raise e
        ...
        if new_token:
            ...
            try:
                ...
                try:
                    ...
                    error_msg = str(e)
                    if "'<' not supported" in error_msg or "NoneType" in error_msg:
                        log(f"🛠 CCXT lỗi nhồi lệnh {new_token}. Fallback Raw API...")
                        raw_resp = raw_create_order(
                            exchange, f"{new_token}/USDT:USDT", 'market', 'buy' if direction == 'long' else 'sell', amount,
                            {'posSide': direction, 'tdMode': 'cross'}
                        )
                        order2 = {'id': raw_resp.get('data', [{}])[0].get('ordId', 'Raw_OK')}
                    else:
                        raise e
            ...
            if min_sum > 40:
                ...
                resume_auto_open_time = base_time + timedelta(minutes=15)
                delay_msg = "15 phút"
